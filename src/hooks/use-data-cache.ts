...
export function useDataCache<T>(
  ...
  // Initialize `loading` to `false` when the hook is disabled so we don't
  // need a setState-in-effect just to flip it back. When the caller toggles
  // `enabled` from true to false, the effect below still re-syncs it.
  ...
  // on every render when the caller uses an inline arrow function. We sync
  // the ref in an effect (not during render) to satisfy react-hooks/refs.
  ...
  useEffect(() => {
    if (!enabled) {
      // `loading` is initialised to `false` when disabled (see useState
      // above), so no setState is needed here.
      return;
    }
