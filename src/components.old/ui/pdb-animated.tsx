...
export function AnimatedNumber({ value, decimals = 0, suffix = '', glowColor }: { value: number; decimals?: number; suffix?: string; glowColor?: string }) {
  ...
  return (
    <motion.span
      className={`tabular-nums tabular-nums-transition inline-block transition-colors duration-300`}
      animate={isAnimating && glowColor ? {
        textShadow: [
          `0 0 0px transparent`,
...
export function TiltCard({ children, className = '', style = {}, animationDelay }: {
  ...
  return (
    <div
      ref={ref}
      className={`tilt-card relative overflow-hidden hover:shadow-lg transition-shadow duration-200 ${className}`}
