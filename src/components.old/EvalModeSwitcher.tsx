...
import { TiltCard } from '@/components/ui/pdb-animated';
                    ...
                    <div className="space-y-1 px-1 pt-1">
                      {filteredEvaluations.length === 0 ? (
                        ...
                        filteredEvaluations.map((evalItem) => {
                          ...
                          return (
                            <TiltCard key={evalItem.uniprotId} className="rounded-md">
                              <button
                                onClick={() => onSelectEval(evalItem.uniprotId)}
                                className={`w-full text-left rounded-md px-3 py-2.5 transition-all duration-150 claude-focus-ring hover:pl-2 ${
                                  isSelected
                        ...
                        filteredBatches.map((batch) => {
                          ...
                          return (
                            <div key={batch.batchId}>
                              <TiltCard className="rounded-md">
                                <button
                                  onClick={() => toggleBatch(batch.batchId)}
                                  className="w-full text-left rounded-md px-3 py-2.5 transition-all duration-150 bg-claude-surface dark:bg-[#242220] border border-transparent hover:border-claude-border dark:hover:border-[#3d3832] hover:bg-claude-border-light dark:hover:bg-[#2b2926] hover:pl-2"
                                >
                                  <div className="flex items-start justify-between gap-2">
                                    <div className="min-w-0 flex-1">
                                      <div className="flex items-center gap-1.5">
                                      ...
                                      <div className="flex items-center gap-2 mt-1">
                                        <span className="text-[9px] text-claude-text-muted">
                                          {batch.subTargetCount} targets
                                        </span>
                                      </div>
                                    </div>
                                    <ChevronRight
                                      className={`h-3.5 w-3.5 text-claude-text-muted transition-transform duration-200 flex-shrink-0 ${
                                      ...
                                      {subTargets.map((st) => {
                                        ...
                                        return (
                                          <button
                                            ...
                                            className={`w-full text-left rounded-md px-2.5 py-1.5 transition-all duration-100 text-[11px] claude-focus-ring hover:pl-2 ${
                                              isSelected
