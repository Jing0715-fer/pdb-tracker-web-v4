...
export function LiteratureContent({
  ...
  const filteredPapers = useMemo(() => {
    ...
    result.sort((a, b) => {
      ...
      switch (sort) {
        case 'IF': {
          // null/undefined 始终排在最后（不论 asc/desc）
  ...
  // ── Source filter (日报) badge in active filters ───────────────────
  // (Added via activeFiltersCount above; visual chip rendered in filter info row)
  ...
  return (
    <>
      ...
      {/* Toolbar Row 2 — Date/IF filter chips, Network, Charts (after separator) — white bg same as Weekly & Evaluation */}
      <div className="px-4 py-2 flex-shrink-0 border-b border-claude-border dark:border-[#3d3832] bg-claude-surface dark:bg-[#242220]">
      ...
      {/* Citation Network — max-h with scrollbar so content below remains accessible */}
      <AnimatePresence>
        {showCitationNetwork && (
          <motion.div
            ...
            className="px-4 flex-shrink-0 max-h-[60vh] overflow-y-auto custom-scrollbar"
          >
            <LiteratureCitationNetwork
              papers={papers}
              onClose={() => setShowCitationNetwork(false)}
              onSelectPaper={(pmid) => {
                // Scroll the corresponding paper card into view and briefly highlight it
      ...
      {/* Charts section — max-h with scrollbar so content below remains accessible */}
      <AnimatePresence>
        {showCharts && stats && (
          <motion.div
            initial={{ opacity: 0, height: 0 }}
