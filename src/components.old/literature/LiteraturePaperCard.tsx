...
import { ExternalLink, BookOpen, ChevronDown, ChevronUp, Link2, Star, Calendar, Users } from 'lucide-react';
...
function getProgressColor(progress: number): string {
  if (progress >= 67) return '#10b981'; // emerald-500
  if (progress >= 34) return '#f59e0b'; // amber-500
  return '#2d8f8f'; // teal (claude-cryo-em)
}
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className={`absolute left-0 top-0 bottom-0 w-[3px] ${ifStyle.bar} transition-all duration-200 group-hover:w-[5px]`} />

        {/* Note indicator dot - small yellow dot in top-right corner */}
        {hasNote && (
          <div className="absolute top-2 right-2 h-2 w-2 rounded-full bg-amber-400 dark:bg-amber-500 ring-2 ring-white dark:ring-[#242220] z-10" title="Has notes" />
        )}

        <div className="pl-3 pr-3 sm:pr-4 py-3 sm:py-4 flex-1 min-w-0">
          {/* Top row: Journal + IF + Action buttons */}
              ...
              {paper.pdbs.slice(0, 6).map((pdb: LitPaperPdb) => {
                ...
                return (
                  <a
                    ...
                    className={`inline-flex items-center gap-1 px-1 sm:px-1.5 py-0.5 rounded text-[8px] sm:text-[9px] font-mono font-medium border transition-all duration-150 hover:scale-105 hover:shadow-sm ${methodStyle.bg} ${methodStyle.text} ${methodStyle.border}`}
                  >
                    {pdb.pdbId}
                    <span className="opacity-60">{getMethodLabel(pdb.method || '')}</span>
                  </a>
                );
              })}
              {paper.pdbs.length > 6 && (
