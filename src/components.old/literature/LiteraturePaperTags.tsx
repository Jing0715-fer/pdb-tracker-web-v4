...
export function TagPill({ tag, onRemove, onClick, size = 'sm' }: TagPillProps) {
  ...
  return (
    <motion.span
      ...
      className={`inline-flex items-center gap-0.5 rounded-md font-medium border ${colors.bg} ${colors.text} ${colors.border} ${sizeClasses} ${
          ...
          {showSuggestions && suggestions.length > 0 && (
            <>
              ...
              <motion.div
                ...
                exit={{ opacity: 0, y: -4 }}
                className="absolute left-0 top-full mt-1 z-20 w-48 max-h-40 overflow-y-auto rounded-lg border border-claude-border dark:border-[#3d3832] bg-white dark:bg-[#2b2926] shadow-lg py-1 custom-scrollbar"
              >
                {suggestions.map(tag => {
                  const colors = TAG_COLORS[tag] || DEFAULT_TAG_COLOR;
                  return (
