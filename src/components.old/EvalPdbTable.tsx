    ...
    <div className="flex flex-col h-full">
      ...
      <div className="flex-1 overflow-auto custom-scrollbar">
        <table className={`w-full border-collapse text-left ${compact ? 'compact-table' : ''}`}>
          <thead className="sticky top-0 z-10 bg-claude-surface dark:bg-[#242220]">
            <tr>
              {EVAL_TABLE_COLUMNS.map((col) => (
                <th
                  key={col.field}
                  className={`table-header-cell px-3 py-2 text-[10px] font-semibold text-claude-text-secondary uppercase tracking-wider whitespace-nowrap ${col.widthClass} ${
                ...
                return (
                  <ContextMenu key={`${row._type}-${row.pdbId}-${idx}`}>
                    <ContextMenuTrigger asChild>
                      <tr
                        className={`table-row-hover cursor-pointer border-l-2 border-l-transparent hover:border-l-claude-accent/40 transition-all duration-150 ${
