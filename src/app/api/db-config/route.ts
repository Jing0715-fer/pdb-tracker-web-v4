...
const TEST_DB_ABS = path.resolve(process.cwd(), 'db', 'custom.db')
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (action === 'init') {
      const activeUrl = resolveDbUrl()
      const r = await initSchema(activeUrl)
      if (!r.ok) {
        return NextResponse.json({ ok: false, error: 'prisma db push failed', stderr: r.stderr }, { status: 500 })
      }
      // Recreate client so the new tables are visible.
      await recreatePrismaClient()
      const schemaInfo = await checkSchema(getActiveDbFsPath())
      return NextResponse.json({
        ok: true,
        message: 'Schema initialized on active database.',
        activeUrl,
        ...schemaInfo,
      })
    }
    ...
    // Initialize schema if requested (default).
    let initResult: { ok: boolean; stderr?: string } | null = null
    if (initSchemaFlag) {
      initResult = await initSchema(normalizedUrl)
    }
    ...
    return NextResponse.json({
      ...
      message: initResult?.ok
        ...
        : `Database path saved to ${fsPath}. Schema init ${initResult?.ok ? 'succeeded' : 'failed — see stderr'}.`,
      ...schemaInfo,
      counts,
    })
  } catch (err: any) {
