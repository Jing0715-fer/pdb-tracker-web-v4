...
export function PdbThumbnailPreview({ pdbId, title, onClick, thumbHeight = 180, hideInfoBar = false }: PdbThumbnailPreviewProps) {
  ...
  // Track the pdbId each piece of state belongs to so we can reset during render
  // when the parent swaps to a different pdbId (React docs: "Adjusting state when a prop changes").
