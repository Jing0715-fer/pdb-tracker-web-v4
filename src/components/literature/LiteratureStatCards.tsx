  ...
  return (
    <div className={`grid grid-cols-2 sm:grid-cols-4${readingProgress ? ' lg:grid-cols-5' : ''} gap-2 sm:gap-3 p-2 sm:p-3 [grid-auto-rows:1fr] stagger-list`}>
      ...
      <StatCard
        ...
        delay={0}
        borderColor="#2d8f8f"
        tooltip={`Total Papers: ${stats.totalPapers} (${stats.papersWithIf} with IF data)`}
      >
      ...
      <StatCard
        ...
        glowColor="#c9872e"
        subtitle={`${stats.ifDistribution.find(d => d.tier === 'top')?.count ?? 0} top-tier`}
        loading={isLoading}
        delay={80}
        borderColor="#c9872e"
        tooltip={`Avg IF: ${(stats.avgIf ?? 0).toFixed(2)}`}
      >
      ...
      <StatCard
        ...
        tooltip={`Top Journal: ${stats.topJournal ?? 'None'}`}
      >
      ...
      <StatCard
        ...
        tooltip={`Latest: ${latestDateInfo.display} (${latestDateInfo.relative})`}
      >
      ...
      {readingProgress && (
        <StatCard
          ...
          glowColor="#2d8f8f"
          subtitle={`${readingProgress.readCount} read · ${readingProgress.readingCount} reading`}
          delay={320}
          borderColor="#2d8f8f"
          tooltip={`Reading: ${readingProgress.progressPercentage.toFixed(0)}% (${readingProgress.readCount} read, ${readingProgress.readingCount} reading)`}
        >
