...
export function LigandInteractionNetwork({
  ...
  const [animatedPositions, setAnimatedPositions] = useState<Record<string, { x: number; y: number; vx: number; vy: number }>>({});
  ...
  useEffect(() => {
    ...
    function animate() {
      ...
      setAnimatedPositions((prev) => {
        ...
        const sim: Record<string, { x: number; y: number; vx: number; vy: number }> = {};
        for (const id of nodeIds) {
          const cur = prev[id] ?? (positions[id]
            ? { x: positions[id].x, y: positions[id].y, vx: 0, vy: 0 }
            : undefined);
          if (!cur) continue;
          sim[id] = { x: cur.x, y: cur.y, vx: cur.vx, vy: cur.vy };
        }
        // Center gravity
        for (const id of nodeIds) {
          if (!sim[id]) continue;
          sim[id].vx += (cx - sim[id].x) * 0.005;
          sim[id].vy += (cy - sim[id].y) * 0.005;
        }
        // Repulsion between nodes
        for (let i = 0; i < nodeIds.length; i++) {
          for (let j = i + 1; j < nodeIds.length; j++) {
            const a = sim[nodeIds[i]];
            const b = sim[nodeIds[j]];
            if (!a || !b) continue;
            const dx = b.x - a.x;
            const dy = b.y - a.y;
        ...
        for (const id of nodeIds) {
          ...
          sim[id].y += sim[id].vy;
          // Boundary clamp
          sim[id].x = Math.max(20, Math.min(svgW - 20, sim[id].x));
          sim[id].y = Math.max(20, Math.min(svgH - 20, sim[id].y));
        }
        return sim;
      });
  ...
  const finalPositions = useMemo(() => {
    ...
    for (const n of nodes) {
      const a = animatedPositions[n.id];
      pos[n.id] = a ? { x: a.x, y: a.y } : (positions[n.id] || { x: cx, y: cy });
    }
    return pos;
  }, [nodes, positions, animatedPositions]);
