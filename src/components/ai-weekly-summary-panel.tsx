...
export function AiWeeklySummaryPanel({ weekId, entries }: AiWeeklySummaryPanelProps) {
  ...
  // Restore cached summary on first render (lazy initializers).
  const initialExpanded = weekId ? !!getCachedSummary(weekId) : false;
  const initialSummary = weekId ? getCachedSummary(weekId) : null;
  ...
  // Re-sync cached summary when weekId changes (idiomatic React: adjust state
  // during render when a prop changes, instead of in an effect).
  const [prevWeekId, setPrevWeekId] = useState(weekId);
  if (weekId !== prevWeekId) {
    setPrevWeekId(weekId);
    if (weekId) {
