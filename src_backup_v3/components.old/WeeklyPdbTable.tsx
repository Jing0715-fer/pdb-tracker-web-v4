...
import { getMethodColor, getMethodLabel, getResolutionColor, getIfTierStyle, formatDate, parseLigands } from '@/components/pdb-helpers';
...
// ─── Resolution Gradient Helper ────────────────────────────────────────────────

function getResolutionGradient(resolution: number | null): string {
  if (resolution === null || resolution === undefined) return '';
  if (resolution < 1.5) return 'from-green-400 to-green-500';
  if (resolution < 2.0) return 'from-green-400 to-amber-400';
  if (resolution < 2.5) return 'from-amber-400 to-amber-500';
  if (resolution < 3.0) return 'from-amber-400 to-orange-400';
  if (resolution < 3.5) return 'from-orange-400 to-red-400';
...
export function WeeklyPdbTable({
  ...
  const renderMethodBadge = (method: string | null) => {
    ...
    return (
      <span className={`method-badge ${badgeClass} inline-flex items-center justify-center min-w-[62px] px-1.5 py-0.5 rounded text-[10px] font-medium border ${colors.bg} ${colors.text} ${colors.border}`}>
        {label}
      </span>
    );
  };
  ...
  const renderResolution = (resolution: number | null) => {
    ...
    const gradient = getResolutionGradient(resolution);
    if (resolution == null) {
      return <span className="font-mono text-[11px] text-claude-text-muted">{'\u2014'}</span>;
    }
    return (
      <div className="relative w-full rounded px-1 py-px overflow-hidden">
        {gradient && (
          <div className={`absolute inset-0 bg-gradient-to-r ${gradient} opacity-15`} />
        )}
        <span className={`relative font-mono text-[11px] font-semibold ${colorClass}`}>
  ...
  const renderIfBadge = (journalIf: number | null, ifTier: string) => {
    ...
    const style = getIfTierStyle(ifTier);
    return (
      <span className={`inline-flex px-1.5 py-0 rounded text-[10px] font-semibold ${style.bg} ${style.text}`}>
        {journalIf.toFixed(1)}
      </span>
    );
  };
    ...
    return (
      <div className="overflow-x-auto">
        <table className="w-full border-collapse text-sm">
          ...
          <tbody>
            <TableSkeleton rows={12} cols={WEEKLY_TABLE_COLUMNS.length + 3} />
          </tbody>
        </table>
      </div>
    );
  ...
  if (entries.length === 0) {
    return (
      <EnhancedEmptyState
        ...
        description="Try adjusting your filters or selecting a different week to find PDB structures."
        accentColor="#2d8f8f"
        suggestions={[
  ...
  return (
    <div className="relative">
      ...
      <div className="overflow-x-auto custom-scrollbar table-scroll-container">
        ...
        <thead>
          <tr className="border-b-2 border-claude-border dark:border-[#4a4540] bg-gradient-to-r from-claude-surface to-[#faf7f4] dark:from-[#242220] dark:to-[#2b2926]">
            ...
            <th className="px-2 py-2 w-10">
              <Checkbox
                ...
                className="[&>[data-state=indeterminate]]:bg-[#c96442] [&>[data-state=indeterminate]]:border-[#c96442] data-[state=checked]:bg-[#c96442] data-[state=checked]:border-[#c96442]"
            ...
            {WEEKLY_TABLE_COLUMNS.map(col => (
              <th
                key={col.field}
                className={`table-header-cell px-3 py-2 text-left text-[11px] font-semibold text-claude-text-secondary whitespace-nowrap ${col.widthClass} ${
            ...
            return (
              <React.Fragment key={entry.pdbId}>
                <motion.tr
                  initial={{ opacity: 0 }}
                  animate={{ opacity: 1 }}
                  transition={{ duration: 0.1, delay: Math.min(idx * 0.01, 0.3) }}
                  className={`cursor-pointer border-b border-claude-border-light dark:border-[#2b2926] transition-all duration-150 border-l-2 border-l-transparent hover:border-l-claude-accent/40 ${
                    isHighlighted
                      ? 'keyboard-focused-row border-l-claude-accent'
                      : isSelected
                        ? 'bg-[#2d8f8f]/8 dark:bg-[#2d8f8f]/12 table-row-hover border-l-[#2d8f8f]/60'
                        : isEven ? 'table-row-even table-row-hover' : 'table-row-odd table-row-hover'
                ...
                <AnimatePresence>
                  {isExpanded && (
                    <tr>
                      <td colSpan={WEEKLY_TABLE_COLUMNS.length + 3} className="p-0 border-b border-claude-border-light dark:border-[#2b2926]">
                        <ExpandedRowDetail 
                          entry={entry} 
